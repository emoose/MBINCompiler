...
namespace MBINCompiler
{
    static class Shared
    {
                    ...
                    case "List`1":
                        ...
                        if (field.FieldType.IsGenericType && field.FieldType.GetGenericTypeDefinition() == typeof(List<>))
                        {
                            ...
                            if (itemType == typeof(NMSTemplate))
                                field.SetValue(obj, reader.DeserializeGenericList());
                            else
                            {
                                ...
                                var list = method.Invoke(null, new object[] { reader });
        ...
        {
            long mod = reader.BaseStream.Position % alignBy;
            if (mod != 0)
                reader.BaseStream.Position += (alignBy - mod);
        }

        public static List<NMSTemplate> DeserializeGenericList(this BinaryReader reader)
        {
            ...
            if (numTemplates > 0)
            {
                ...
                {
                    long unk = reader.ReadInt64();
                    var name = reader.ReadChars(0x40);
                    templateNames.Add(new string(name).TrimEnd(new[] { '\0' }));
                }
        ...
        public static List<T> DeserializeList<T>(this BinaryReader reader)
        {
            long listPosition = reader.BaseStream.Position;
